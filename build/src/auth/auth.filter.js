"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SupertokensExceptionFilter = void 0;
var common_1 = require("@nestjs/common");
var express_1 = require("supertokens-node/framework/express");
var supertokens_node_1 = require("supertokens-node");
var SupertokensExceptionFilter = (function () {
    function SupertokensExceptionFilter() {
        this.handler = (0, express_1.errorHandler)();
    }
    SupertokensExceptionFilter.prototype.catch = function (exception, host) {
        var ctx = host.switchToHttp();
        var resp = ctx.getResponse();
        if (resp.headersSent) {
            return;
        }
        this.handler(exception, ctx.getRequest(), resp, ctx.getNext());
    };
    SupertokensExceptionFilter = __decorate([
        (0, common_1.Catch)(supertokens_node_1.Error),
        __metadata("design:paramtypes", [])
    ], SupertokensExceptionFilter);
    return SupertokensExceptionFilter;
}());
exports.SupertokensExceptionFilter = SupertokensExceptionFilter;
//# sourceMappingURL=auth.filter.js.map